...
class BatchNormalization(Module):
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        if self.training:
            dl_dz = grad_output * self.inv_sqrt_var + np.sum(grad_output 
            * self.input_mean, axis=0) * (-self.inv_sqrt_var ** 2) \
            * self.inv_sqrt_var / 2 / B * 2 * self.input_mean
            return dl_dz + np.sum(-dl_dz, axis=0) / B
