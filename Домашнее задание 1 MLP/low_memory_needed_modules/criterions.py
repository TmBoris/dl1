...
class CrossEntropyLoss(Criterion):
    ...
    def compute_output(self, input: np.ndarray, target: np.ndarray) -> float:
        ...
        self.mask[tuple(np.arange(len(target))), tuple(target)] = 1
        return -(np.sum(self.log_softmax.compute_output(input) * self.mask) / input.shape[0])
    ...
    def compute_grad_input(self, input: np.ndarray, target: np.ndarray) -> np.ndarray:
        ...
        """
        return self.log_softmax.compute_grad_input(input, -self.mask / len(target))
